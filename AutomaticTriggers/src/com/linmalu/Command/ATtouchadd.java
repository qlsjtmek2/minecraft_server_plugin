...
public class ATtouchadd implements CommandExecutor
{
	private AutomaticTriggers plugin;

	public ATtouchadd(AutomaticTriggers plugin)
	{
		this.plugin = plugin;
	}
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args)
	{
		if(sender.hasPermission("atriggers.admin") || sender.isOp() == true)
		{
			...
			if (args.length > 1)
			{
				...
				if(plugin.scriptCommands.EqualScriptCommand(sender, args, "touchadd", line))
					return true;
				plugin.listener.setListener(sender.getName(), args[1], plugin.ScriptLine(args, 1), line, "touchadd", true);
