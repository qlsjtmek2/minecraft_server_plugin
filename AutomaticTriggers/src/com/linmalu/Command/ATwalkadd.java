...
public class ATwalkadd implements CommandExecutor
{
	private AutomaticTriggers plugin;

	public ATwalkadd(AutomaticTriggers plugin)
	{
		this.plugin = plugin;
	}
		...
		if(sender.hasPermission("atriggers.admin") || sender.isOp() == true)
		{
			...
			if (args.length > 1)
			{
				...
				if(plugin.scriptCommands.EqualScriptCommand(sender, args, "walk", line))
					return true;
				plugin.listener.setListener(sender.getName(), args[1], plugin.ScriptLine(args, 1), line, "walkadd", true);
