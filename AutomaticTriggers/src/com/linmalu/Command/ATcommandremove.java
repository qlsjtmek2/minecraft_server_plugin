...
public class ATcommandremove implements CommandExecutor
{
	private AutomaticTriggers plugin;

	public ATcommandremove(AutomaticTriggers plugin)
	{
		this.plugin = plugin;
	}
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args)
	{
		if(sender.hasPermission("atriggers.admin") || sender.isOp() == true)
		{
			if(args.length == 2)
			{
				...
				cmds.removeScripts(args[0], line);
				sender.sendMessage(ChatColor.YELLOW + args[0] + " 명령어의 " + ChatColor.GREEN + line + "번째줄이 삭제되었습니다.");
				return true;
			}
		}
		else
