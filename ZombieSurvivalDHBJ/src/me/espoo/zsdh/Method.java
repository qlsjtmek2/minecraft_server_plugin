...
public class Method extends JavaPlugin {
		...
		for (String la : list) {
			...
			if (Method.getInfoBoolean(p, "Observer") == false) {
				...
				if (la.equals(ZombieName)) {
					...
					main.zombieData.put(p.getName(), "숙주");
					p.getInventory().clear();
					...
					if (main.zombieData.get(p.getName()) != null) main.zombieData.remove(p.getName());
					if (main.humanData.get(p.getName()) != null) main.humanData.remove(p.getName());
					main.humanData.put(p.getName(), "true");
	...
	public static void GameStop() {
		...
		for (String la : list) {
			...
			if (main.humanData.get(p.getName()) != null) {
				...
				main.zombieData.remove(p.getName()); }
		}
	}
	...
	public static void openGunGUI(Player p) {
		...
		Bukkit.getServer().dispatchCommand(Bukkit.getServer().getConsoleSender(), "chc open SGun " + p.getName());
	}
