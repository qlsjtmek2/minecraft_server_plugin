...
public class Deposit {
	public static void CreateDepositConfig(File f, File folder, YamlConfiguration config)
	{
		try
		{
			folder.mkdir();
			f.createNewFile();
            config.set("알림문", "&a클릭시 투자 자금을 획득합니다.");
            config.set("default.이름", "&f현재 가능한 투자가 없습니다.");
    		List<String> list = config.getStringList("default.설명");
            list.add("&a당신은 모든 투자를 완료했습니다.");
            config.set("default.설명", list);
            config.set("default.아이템 코드", 339);
            config.set("default.아이템 데이터코드", 0);
            config.set("default.아이템 수량", 1);
            config.set("1.이름", "코리아증권투자");
            config.set("1.투자가능 금액", 100);
            config.set("1.만기 투자금", 120);
            config.set("1.만기시간 (분)", 20);
            config.set("1.이율", "20%");
            config.set("2.이름", "동부파워증권투자");
            config.set("2.투자가능 금액", 150);
            config.set("2.만기 투자금", 180);
            config.set("2.만기시간 (분)", 25);
            config.set("2.이율", "20%");
            config.set("3.이름", "BNK골드스코프증권투자");
            config.set("3.투자가능 금액", 200);
            config.set("3.만기 투자금", 250);
	...
	public static List<String> getLoreList(Player p, String name) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		if (!f.exists()) {
			CreateDepositConfig(f, folder, config);
		}
	...
	public static Boolean getDepositBoolean(String name) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		if (!f.exists()) {
	...
	public static int getDepositInt(String name) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		if (!f.exists()) {
	...
	public static String getDepositString(String name) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		if (!f.exists()) {
	...
	public static void setDepositBoolean(String name, Boolean amount) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		try {
	...
	public static void setDepositInt(String name, int amount) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		try {
	...
	public static void setDepositString(String name, String amount) {
		File f = new File("plugins/DHFinance/Deposit.yml");
		File folder = new File("plugins/DHFinance");
		YamlConfiguration config = YamlConfiguration.loadConfiguration(f);
		try {
