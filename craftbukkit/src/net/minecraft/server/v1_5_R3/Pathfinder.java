...
public class Pathfinder
{
    ...
    private PathEntity a(final Entity entity, final double n, final double n2, final double n3, final float n4) {
        ...
        if (this.h && entity.G()) {
            ...
            for (int n5 = this.a.getTypeId(MathHelper.floor(entity.locX), floor, MathHelper.floor(entity.locZ)); n5 == Block.WATER.id || n5 == Block.STATIONARY_WATER.id; n5 = this.a.getTypeId(MathHelper.floor(entity.locX), floor, MathHelper.floor(entity.locZ))) {
                ++floor;
            }
            g = this.g;
            this.g = false;
        ...
        final PathEntity a = this.a(entity, this.a(MathHelper.floor(entity.boundingBox.a), floor, MathHelper.floor(entity.boundingBox.c)), this.a(MathHelper.floor(n - entity.width / 2.0f), MathHelper.floor(n2), MathHelper.floor(n3 - entity.width / 2.0f)), new PathPoint(MathHelper.d(entity.width + 1.0f), MathHelper.d(entity.length + 1.0f), MathHelper.d(entity.width + 1.0f)), n4);
        this.g = g;
        return a;
    }
        ...
        for (int i = n; i < n + pathPoint.a; ++i) {
            for (int j = n2; j < n2 + pathPoint.b; ++j) {
                for (int k = n3; k < n3 + pathPoint.c; ++k) {
                    final int typeId = entity.world.getTypeId(i, j, k);
                    if (typeId > 0) {
                        if (typeId == Block.TRAP_DOOR.id) {
                            b4 = true;
                        }
                        else if (typeId == Block.WATER.id || typeId == Block.STATIONARY_WATER.id) {
                            if (b) {
                                return -1;
                            }
                            b4 = true;
                        }
                        else if (!b3 && typeId == Block.WOODEN_DOOR.id) {
                            return 0;
                        }
                        final Block block = Block.byId[typeId];
                        final int d = block.d();
                        if (entity.world.e(i, j, k) == 9) {
                            ...
                            if (entity.world.e(floor, floor2, floor3) != 9) {
                                if (entity.world.e(floor, floor2 - 1, floor3) != 9) {
                                    return -3;
                                }
                            }
                        }
                        else if (!block.b((IBlockAccess)entity.world, i, j, k)) {
                            if (!b2 || typeId != Block.WOODEN_DOOR.id) {
                                if (d == 11 || typeId == Block.FENCE_GATE.id || d == 32) {
                                    return -3;
                                }
                                if (typeId == Block.TRAP_DOOR.id) {
                                    return -4;
