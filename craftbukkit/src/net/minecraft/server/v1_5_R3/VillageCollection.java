...
public class VillageCollection extends WorldMapBase
{
    ...
    public VillageCollection(final World world) {
        super("villages");
        this.b = new ArrayList();
        this.c = new ArrayList();
        this.villages = new ArrayList();
    ...
    public Village getClosestVillage(final int n, final int n2, final int n3, final int n4) {
        ...
        for (final Village village2 : this.villages) {
            ...
            final int n6 = n4 + village2.getSize();
            if (e > n6 * n6) {
                continue;
            }
            village = village2;
    ...
    private void c(final int i, final int j, final int k) {
        final int d = ((BlockDoor)Block.WOODEN_DOOR).d(this.world, i, j, k);
        if (d == 0 || d == 2) {
            int n = 0;
            for (int l = -5; l < 0; ++l) {
                if (this.world.l(i + l, j, k)) {
                    --n;
                }
            }
            for (int n2 = 1; n2 <= 5; ++n2) {
                if (this.world.l(i + n2, j, k)) {
                    ++n;
                }
            }
            if (n != 0) {
            ...
            for (int n4 = -5; n4 < 0; ++n4) {
                if (this.world.l(i, j, k + n4)) {
                    --n3;
                }
            }
            for (int n5 = 1; n5 <= 5; ++n5) {
                if (this.world.l(i, j, k + n5)) {
                    ++n3;
                }
            }
            if (n3 != 0) {
    ...
    private boolean e(final int i, final int j, final int k) {
        return this.world.getTypeId(i, j, k) == Block.WOODEN_DOOR.id;
    }
    
    public void a(final NBTTagCompound nbtTagCompound) {
        this.time = nbtTagCompound.getInt("Tick");
        final NBTTagList list = nbtTagCompound.getList("Villages");
    ...
    public void b(final NBTTagCompound nbtTagCompound) {
        ...
        final NBTTagList list = new NBTTagList("Villages");
        for (final Village village : this.villages) {
            final NBTTagCompound nbtTagCompound2 = new NBTTagCompound("Village");
