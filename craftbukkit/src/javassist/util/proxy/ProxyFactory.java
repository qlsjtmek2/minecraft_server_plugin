...
public class ProxyFactory
{
    ...
    private void makeSortedMethodList() {
        ...
        this.hasGetHandler = (allMethods.get("getHandler:()") != null);
        Collections.sort((List<Object>)this.signatureMethods, ProxyFactory.sorter);
    }
    ...
    private static void getMethods(final HashMap hash, final Class clazz, final Set visitedClasses) {
        ...
        for (int j = 0; j < methods.length; ++j) {
            if (!Modifier.isPrivate(methods[j].getModifiers())) {
                ...
                final String key = m.getName() + ':' + RuntimeSupport.makeDescriptor(m.getParameterTypes(), null);
    ...
    private static String keyToDesc(final String key, final Method m) {
        final String params = key.substring(key.indexOf(58) + 1);
