...
public class LongObjectHashMap<V> implements Cloneable, Serializable
{
    ...
    public V put(final long key, final V value) {
        ...
        V[] innerValues = (V[])this.values[index];
        ++this.modCount;
        if (innerKeys == null) {
            innerKeys = (this.keys[index] = new long[8]);
            Arrays.fill(innerKeys, Long.MIN_VALUE);
            innerValues = (V[])(this.values[index] = new Object[8]);
            innerKeys[0] = key;
            innerValues[0] = value;
            ++this.size;
        }
            ...
            innerKeys = (this.keys[index] = Java15Compat.Arrays_copyOf(innerKeys, i << 1));
            Arrays.fill(innerKeys, i, innerKeys.length, Long.MIN_VALUE);
            innerValues = (V[])(this.values[index] = Java15Compat.Arrays_copyOf(innerValues, i << 1));
            innerKeys[i] = key;
            innerValues[i] = value;
            ++this.size;
        }
