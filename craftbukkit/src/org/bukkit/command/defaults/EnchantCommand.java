...
{
    private static final List<String> ENCHANTMENT_NAMES;
    ...
    public boolean execute(final CommandSender sender, final String commandLabel, final String[] args) {
        ...
        if (player == null) {
            ...
            if (item.getType() == Material.AIR) {
                ...
                final Enchantment enchantment = this.getEnchantment(args[1].toUpperCase());
                if (enchantment == null) {
                    ...
                    if (!force && !enchantment.canEnchantItem(item)) {
                        ...
                        int level = 1;
                        if (args.length > 2) {
                            final Integer integer = this.getInteger(args[2]);
                            final int minLevel = enchantment.getStartLevel();
                            final int maxLevel = force ? 32767 : enchantment.getMaxLevel();
                            if (integer != null) {
                                if (integer == 0) {
    ...
    public List<String> tabComplete(final CommandSender sender, final String alias, final String[] args) throws IllegalArgumentException {
        ...
        if (args.length == 2) {
            return StringUtil.copyPartialMatches(args[1], EnchantCommand.ENCHANTMENT_NAMES, new ArrayList<String>(EnchantCommand.ENCHANTMENT_NAMES.size()));
        }
        if ((args.length == 3 || args.length == 4) && !args[args.length - 2].equalsIgnoreCase("force")) {
    ...
    private Enchantment getEnchantment(final String lookup) {
        ...
        if (enchantment == null) {
            final Integer id = this.getInteger(lookup);
            if (id != null) {
    ...
    static {
        ENCHANTMENT_NAMES = new ArrayList<String>();
    }
}
