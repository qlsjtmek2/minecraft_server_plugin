...
public class PerlinNoiseGenerator extends NoiseGenerator
{
    ...
    public double noise(double x, double y, double z) {
        ...
        y += this.offsetY;
        z += this.offsetZ;
        final int floorX = NoiseGenerator.floor(x);
        final int floorY = NoiseGenerator.floor(y);
        final int floorZ = NoiseGenerator.floor(z);
        ...
        y -= floorY;
        z -= floorZ;
        final double fX = NoiseGenerator.fade(x);
        final double fY = NoiseGenerator.fade(y);
        final double fZ = NoiseGenerator.fade(z);
        ...
        return NoiseGenerator.lerp(fZ, NoiseGenerator.lerp(fY, NoiseGenerator.lerp(fX, NoiseGenerator.grad(this.perm[AA], x, y, z), NoiseGenerator.grad(this.perm[BA], x - 1.0, y, z)), NoiseGenerator.lerp(fX, NoiseGenerator.grad(this.perm[AB], x, y - 1.0, z), NoiseGenerator.grad(this.perm[BB], x - 1.0, y - 1.0, z))), NoiseGenerator.lerp(fY, NoiseGenerator.lerp(fX, NoiseGenerator.grad(this.perm[AA + 1], x, y, z - 1.0), NoiseGenerator.grad(this.perm[BA + 1], x - 1.0, y, z - 1.0)), NoiseGenerator.lerp(fX, NoiseGenerator.grad(this.perm[AB + 1], x, y - 1.0, z - 1.0), NoiseGenerator.grad(this.perm[BB + 1], x - 1.0, y - 1.0, z - 1.0))));
    }
