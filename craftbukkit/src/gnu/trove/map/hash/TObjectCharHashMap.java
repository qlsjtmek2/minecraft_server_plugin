...
public class TObjectCharHashMap<K> extends TObjectHash<K> implements TObjectCharMap<K>, Externalizable
{
    ...
    public TObjectCharHashMap(final int initialCapacity) {
    ...
    public TObjectCharHashMap(final int initialCapacity, final float loadFactor) {
    ...
    public TObjectCharHashMap(final int initialCapacity, final float loadFactor, final char noEntryValue) {
        super(initialCapacity, loadFactor);
        this.PUT_ALL_PROC = new TObjectCharProcedure<K>() {
            public boolean execute(final K key, final char value) {
                TObjectCharHashMap.this.put(key, value);
    ...
    public TObjectCharHashMap(final TObjectCharMap<? extends K> map) {
        this(map.size(), 0.5f, map.getNoEntryValue());
        if (map instanceof TObjectCharHashMap) {
            final TObjectCharHashMap hashmap = (TObjectCharHashMap)map;
    ...
    protected void rehash(final int newCapacity) {
        ...
        Arrays.fill(this._set = new Object[newCapacity], TObjectCharHashMap.FREE);
        Arrays.fill(this._values = new char[newCapacity], this.no_entry_value);
        int i = oldCapacity;
        while (i-- > 0) {
            if (oldKeys[i] != TObjectCharHashMap.FREE && oldKeys[i] != TObjectCharHashMap.REMOVED) {
                final K o = oldKeys[i];
    ...
    public boolean containsValue(final char val) {
        ...
        int i = vals.length;
        while (i-- > 0) {
            if (keys[i] != TObjectCharHashMap.FREE && keys[i] != TObjectCharHashMap.REMOVED && val == vals[i]) {
                return true;
            }
        }
        return false;
    ...
    public void clear() {
        super.clear();
        Arrays.fill(this._set, 0, this._set.length, TObjectCharHashMap.FREE);
    ...
    public Object[] keys() {
        ...
        int j = 0;
        while (i-- > 0) {
            if (k[i] != TObjectCharHashMap.FREE && k[i] != TObjectCharHashMap.REMOVED) {
                keys[j++] = (K)k[i];
            }
        }
        return keys;
    ...
    public char[] values() {
        ...
        int j = 0;
        while (i-- > 0) {
            if (keys[i] != TObjectCharHashMap.FREE && keys[i] != TObjectCharHashMap.REMOVED) {
                vals[j++] = v[i];
            }
        }
        return vals;
    ...
    public boolean forEachValue(final TCharProcedure procedure) {
        ...
        int i = values.length;
        while (i-- > 0) {
            if (keys[i] != TObjectCharHashMap.FREE && keys[i] != TObjectCharHashMap.REMOVED && !procedure.execute(values[i])) {
                return false;
            }
        }
        return true;
    ...
    public boolean forEachEntry(final TObjectCharProcedure<? super K> procedure) {
        ...
        int i = keys.length;
        while (i-- > 0) {
            if (keys[i] != TObjectCharHashMap.FREE && keys[i] != TObjectCharHashMap.REMOVED && !procedure.execute((Object)keys[i], values[i])) {
                return false;
            }
        }
        return true;
    ...
    public void transformValues(final TCharFunction function) {
        ...
        int i = values.length;
        while (i-- > 0) {
            if (keys[i] != null && keys[i] != TObjectCharHashMap.REMOVED) {
    ...
    public void writeExternal(final ObjectOutput out) throws IOException {
        ...
        while (i-- > 0) {
            if (this._set[i] != TObjectCharHashMap.REMOVED && this._set[i] != TObjectCharHashMap.FREE) {
    ...
    class TCharValueCollection implements TCharCollection
    {
        ...
        public boolean remove(final char entry) {
            final char[] values = TObjectCharHashMap.this._values;
            final Object[] set = TObjectCharHashMap.this._set;
            int i = values.length;
            while (i-- > 0) {
    ...
    class TObjectCharHashIterator<K> extends TObjectHashIterator<K> implements TObjectCharIterator<K>
    {
        ...
        public TObjectCharHashIterator(final TObjectCharHashMap<K> map) {
            super(map);
            this._map = map;
        }
