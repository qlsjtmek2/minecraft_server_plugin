...
public class TObjectShortHashMap<K> extends TObjectHash<K> implements TObjectShortMap<K>, Externalizable
{
    ...
    public TObjectShortHashMap(final int initialCapacity) {
    ...
    public TObjectShortHashMap(final int initialCapacity, final float loadFactor) {
    ...
    public TObjectShortHashMap(final int initialCapacity, final float loadFactor, final short noEntryValue) {
        super(initialCapacity, loadFactor);
        this.PUT_ALL_PROC = new TObjectShortProcedure<K>() {
            public boolean execute(final K key, final short value) {
                TObjectShortHashMap.this.put(key, value);
    ...
    public TObjectShortHashMap(final TObjectShortMap<? extends K> map) {
        this(map.size(), 0.5f, map.getNoEntryValue());
        if (map instanceof TObjectShortHashMap) {
            final TObjectShortHashMap hashmap = (TObjectShortHashMap)map;
    ...
    protected void rehash(final int newCapacity) {
        ...
        Arrays.fill(this._set = new Object[newCapacity], TObjectShortHashMap.FREE);
        Arrays.fill(this._values = new short[newCapacity], this.no_entry_value);
        int i = oldCapacity;
        while (i-- > 0) {
            if (oldKeys[i] != TObjectShortHashMap.FREE && oldKeys[i] != TObjectShortHashMap.REMOVED) {
                final K o = oldKeys[i];
    ...
    public boolean containsValue(final short val) {
        ...
        int i = vals.length;
        while (i-- > 0) {
            if (keys[i] != TObjectShortHashMap.FREE && keys[i] != TObjectShortHashMap.REMOVED && val == vals[i]) {
                return true;
            }
        }
        return false;
    ...
    public void clear() {
        super.clear();
        Arrays.fill(this._set, 0, this._set.length, TObjectShortHashMap.FREE);
    ...
    public Object[] keys() {
        ...
        int j = 0;
        while (i-- > 0) {
            if (k[i] != TObjectShortHashMap.FREE && k[i] != TObjectShortHashMap.REMOVED) {
                keys[j++] = (K)k[i];
            }
        }
        return keys;
    ...
    public short[] values() {
        ...
        int j = 0;
        while (i-- > 0) {
            if (keys[i] != TObjectShortHashMap.FREE && keys[i] != TObjectShortHashMap.REMOVED) {
                vals[j++] = v[i];
            }
        }
        return vals;
    ...
    public boolean forEachValue(final TShortProcedure procedure) {
        ...
        int i = values.length;
        while (i-- > 0) {
            if (keys[i] != TObjectShortHashMap.FREE && keys[i] != TObjectShortHashMap.REMOVED && !procedure.execute(values[i])) {
                return false;
            }
        }
        return true;
    ...
    public boolean forEachEntry(final TObjectShortProcedure<? super K> procedure) {
        ...
        int i = keys.length;
        while (i-- > 0) {
            if (keys[i] != TObjectShortHashMap.FREE && keys[i] != TObjectShortHashMap.REMOVED && !procedure.execute((Object)keys[i], values[i])) {
                return false;
            }
        }
        return true;
    ...
    public void transformValues(final TShortFunction function) {
        ...
        int i = values.length;
        while (i-- > 0) {
            if (keys[i] != null && keys[i] != TObjectShortHashMap.REMOVED) {
    ...
    public void writeExternal(final ObjectOutput out) throws IOException {
        ...
        while (i-- > 0) {
            if (this._set[i] != TObjectShortHashMap.REMOVED && this._set[i] != TObjectShortHashMap.FREE) {
    ...
    class TShortValueCollection implements TShortCollection
    {
        ...
        public boolean remove(final short entry) {
            final short[] values = TObjectShortHashMap.this._values;
            final Object[] set = TObjectShortHashMap.this._set;
            int i = values.length;
            while (i-- > 0) {
    ...
    class TObjectShortHashIterator<K> extends TObjectHashIterator<K> implements TObjectShortIterator<K>
    {
        ...
        public TObjectShortHashIterator(final TObjectShortHashMap<K> map) {
            super(map);
            this._map = map;
        }
