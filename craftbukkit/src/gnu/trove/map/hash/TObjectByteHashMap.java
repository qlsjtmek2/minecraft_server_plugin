...
public class TObjectByteHashMap<K> extends TObjectHash<K> implements TObjectByteMap<K>, Externalizable
{
    ...
    public TObjectByteHashMap(final int initialCapacity, final float loadFactor, final byte noEntryValue) {
        super(initialCapacity, loadFactor);
        this.PUT_ALL_PROC = new TObjectByteProcedure<K>() {
            public boolean execute(final K key, final byte value) {
                TObjectByteHashMap.this.put(key, value);
    ...
    public TObjectByteHashMap(final TObjectByteMap<? extends K> map) {
        this(map.size(), 0.5f, map.getNoEntryValue());
        if (map instanceof TObjectByteHashMap) {
            final TObjectByteHashMap hashmap = (TObjectByteHashMap)map;
    ...
    public boolean forEachEntry(final TObjectByteProcedure<? super K> procedure) {
        ...
        int i = keys.length;
        while (i-- > 0) {
            if (keys[i] != TObjectByteHashMap.FREE && keys[i] != TObjectByteHashMap.REMOVED && !procedure.execute((Object)keys[i], values[i])) {
                return false;
            }
        }
        return true;
    ...
    public void writeExternal(final ObjectOutput out) throws IOException {
        ...
        while (i-- > 0) {
            if (this._set[i] != TObjectByteHashMap.REMOVED && this._set[i] != TObjectByteHashMap.FREE) {
    ...
    protected class KeyView extends MapBackedView<K>
    {
        public Iterator<K> iterator() {
            return new TObjectHashIterator<K>(TObjectByteHashMap.this);
        }
    ...
    class TByteValueCollection implements TByteCollection
    {
        ...
        public boolean remove(final byte entry) {
            final byte[] values = TObjectByteHashMap.this._values;
            final Object[] set = TObjectByteHashMap.this._set;
            int i = values.length;
            while (i-- > 0) {
    ...
    class TObjectByteHashIterator<K> extends TObjectHashIterator<K> implements TObjectByteIterator<K>
    {
        ...
        public TObjectByteHashIterator(final TObjectByteHashMap<K> map) {
            super(map);
            this._map = map;
        }
