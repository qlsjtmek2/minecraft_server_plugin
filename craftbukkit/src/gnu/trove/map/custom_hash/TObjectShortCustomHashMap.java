...
public class TObjectShortCustomHashMap<K> extends TCustomObjectHash<K> implements TObjectShortMap<K>, Externalizable
{
    ...
    public TObjectShortCustomHashMap() {
        this.PUT_ALL_PROC = new TObjectShortProcedure<K>() {
            public boolean execute(final K key, final short value) {
    ...
    public TObjectShortCustomHashMap(final HashingStrategy<? super K> strategy) {
        super(strategy);
        this.PUT_ALL_PROC = new TObjectShortProcedure<K>() {
            public boolean execute(final K key, final short value) {
                TObjectShortCustomHashMap.this.put(key, value);
    ...
    public TObjectShortCustomHashMap(final HashingStrategy<? super K> strategy, final int initialCapacity) {
    ...
    public TObjectShortCustomHashMap(final HashingStrategy<? super K> strategy, final int initialCapacity, final float loadFactor) {
    ...
    public TObjectShortCustomHashMap(final HashingStrategy<? super K> strategy, final int initialCapacity, final float loadFactor, final short noEntryValue) {
        super(strategy, initialCapacity, loadFactor);
        this.PUT_ALL_PROC = new TObjectShortProcedure<K>() {
            public boolean execute(final K key, final short value) {
                TObjectShortCustomHashMap.this.put(key, value);
    ...
    public TObjectShortCustomHashMap(final HashingStrategy<? super K> strategy, final TObjectShortMap<? extends K> map) {
        this(strategy, map.size(), 0.5f, map.getNoEntryValue());
        if (map instanceof TObjectShortCustomHashMap) {
            final TObjectShortCustomHashMap hashmap = (TObjectShortCustomHashMap)map;
    ...
    protected void rehash(final int newCapacity) {
        ...
        Arrays.fill(this._set = new Object[newCapacity], TObjectShortCustomHashMap.FREE);
        Arrays.fill(this._values = new short[newCapacity], this.no_entry_value);
        int i = oldCapacity;
        while (i-- > 0) {
            final K o = oldKeys[i];
            if (o != TObjectShortCustomHashMap.FREE && o != TObjectShortCustomHashMap.REMOVED) {
    ...
    public boolean containsValue(final short val) {
        ...
        int i = vals.length;
        while (i-- > 0) {
            if (keys[i] != TObjectShortCustomHashMap.FREE && keys[i] != TObjectShortCustomHashMap.REMOVED && val == vals[i]) {
                return true;
            }
        }
        return false;
    ...
    private short doPut(final short value, int index) {
    ...
    public void putAll(final Map<? extends K, ? extends Short> map) {
        final Set<? extends Map.Entry<? extends K, ? extends Short>> set = map.entrySet();
        for (final Map.Entry<? extends K, ? extends Short> entry : set) {
            this.put(entry.getKey(), (short)entry.getValue());
        }
    }
    ...
    public void clear() {
        super.clear();
        Arrays.fill(this._set, 0, this._set.length, TObjectShortCustomHashMap.FREE);
    ...
    public Object[] keys() {
        ...
        int j = 0;
        while (i-- > 0) {
            if (k[i] != TObjectShortCustomHashMap.FREE && k[i] != TObjectShortCustomHashMap.REMOVED) {
                keys[j++] = (K)k[i];
            }
        }
        return keys;
    ...
    public short[] values() {
        ...
        int j = 0;
        while (i-- > 0) {
            if (keys[i] != TObjectShortCustomHashMap.FREE && keys[i] != TObjectShortCustomHashMap.REMOVED) {
                vals[j++] = v[i];
            }
        }
        return vals;
    ...
    public boolean forEachValue(final TShortProcedure procedure) {
        ...
        int i = values.length;
        while (i-- > 0) {
            if (keys[i] != TObjectShortCustomHashMap.FREE && keys[i] != TObjectShortCustomHashMap.REMOVED && !procedure.execute(values[i])) {
                return false;
            }
        }
        return true;
    ...
    public boolean forEachEntry(final TObjectShortProcedure<? super K> procedure) {
        ...
        int i = keys.length;
        while (i-- > 0) {
            if (keys[i] != TObjectShortCustomHashMap.FREE && keys[i] != TObjectShortCustomHashMap.REMOVED && !procedure.execute((Object)keys[i], values[i])) {
                return false;
            }
        }
        return true;
    ...
    public void transformValues(final TShortFunction function) {
        ...
        int i = values.length;
        while (i-- > 0) {
            if (keys[i] != null && keys[i] != TObjectShortCustomHashMap.REMOVED) {
    ...
    public boolean equals(final Object other) {
        ...
        final TObjectShortMap that = (TObjectShortMap)other;
        if (that.size() != this.size()) {
            return false;
        }
        try {
            final TObjectShortIterator iter = this.iterator();
            while (iter.hasNext()) {
                iter.advance();
                final Object key = iter.key();
                final short value = iter.value();
    ...
    public void writeExternal(final ObjectOutput out) throws IOException {
        ...
        while (i-- > 0) {
            if (this._set[i] != TObjectShortCustomHashMap.REMOVED && this._set[i] != TObjectShortCustomHashMap.FREE) {
    ...
    protected class KeyView extends MapBackedView<K>
    {
        public Iterator<K> iterator() {
            return new TObjectHashIterator<K>(TObjectShortCustomHashMap.this);
        }
    ...
    class TShortValueCollection implements TShortCollection
    {
        ...
        public boolean remove(final short entry) {
            final short[] values = TObjectShortCustomHashMap.this._values;
            final Object[] set = TObjectShortCustomHashMap.this._set;
            int i = values.length;
            while (i-- > 0) {
    ...
    class TObjectShortHashIterator<K> extends TObjectHashIterator<K> implements TObjectShortIterator<K>
    {
        ...
        public TObjectShortHashIterator(final TObjectShortCustomHashMap<K> map) {
            super(map);
            this._map = map;
        }
