...
public abstract class TDoubleDoubleHash extends TPrimitiveHash
{
    ...
    public TDoubleDoubleHash(final int initialCapacity) {
        super(initialCapacity);
        this.no_entry_key = 0.0;
        this.no_entry_value = 0.0;
    }
    
    public TDoubleDoubleHash(final int initialCapacity, final float loadFactor) {
        super(initialCapacity, loadFactor);
        this.no_entry_key = 0.0;
        this.no_entry_value = 0.0;
    }
    
    public TDoubleDoubleHash(final int initialCapacity, final float loadFactor, final double no_entry_key, final double no_entry_value) {
        super(initialCapacity, loadFactor);
    ...
    protected int index(final double key) {
        final byte[] states = this._states;
        final double[] set = this._set;
    ...
    int indexRehashed(final double key, int index, final int hash, byte state) {
    ...
    int insertKeyRehash(final double val, int index, final int hash, byte state) {
        final int length = this._set.length;
        final int probe = 1 + hash % (length - 2);
        final int loopIndex = index;
        int firstRemoved = -1;
    ...
    void insertKeyAt(final int index, final double val) {
        this._set[index] = val;
        this._states[index] = 1;
    }
    
    protected int XinsertKey(final double key) {
        final byte[] states = this._states;
        final double[] set = this._set;
        final int length = states.length;
        final int hash = HashFunctions.hash(key) & Integer.MAX_VALUE;
