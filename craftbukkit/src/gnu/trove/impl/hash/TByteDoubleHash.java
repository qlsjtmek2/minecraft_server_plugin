...
public abstract class TByteDoubleHash extends TPrimitiveHash
{
    ...
    public TByteDoubleHash(final int initialCapacity) {
        super(initialCapacity);
        this.no_entry_key = 0;
        this.no_entry_value = 0.0;
    }
    
    public TByteDoubleHash(final int initialCapacity, final float loadFactor) {
        super(initialCapacity, loadFactor);
        this.no_entry_key = 0;
        this.no_entry_value = 0.0;
    }
    
    public TByteDoubleHash(final int initialCapacity, final float loadFactor, final byte no_entry_key, final double no_entry_value) {
        super(initialCapacity, loadFactor);
    ...
    protected int index(final byte key) {
        final byte[] states = this._states;
        final byte[] set = this._set;
    ...
    int indexRehashed(final byte key, int index, final int hash, byte state) {
    ...
    protected int insertKey(final byte val) {
    ...
    int insertKeyRehash(final byte val, int index, final int hash, byte state) {
        final int length = this._set.length;
        final int probe = 1 + hash % (length - 2);
        final int loopIndex = index;
        int firstRemoved = -1;
    ...
    void insertKeyAt(final int index, final byte val) {
        this._set[index] = val;
        this._states[index] = 1;
    }
    
    protected int XinsertKey(final byte key) {
        final byte[] states = this._states;
        final byte[] set = this._set;
        final int length = states.length;
        final int hash = HashFunctions.hash(key) & Integer.MAX_VALUE;
