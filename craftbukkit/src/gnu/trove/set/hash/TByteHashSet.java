...
public class TByteHashSet extends TByteHash implements TByteSet, Externalizable
{
    ...
    public TByteHashSet(final int initialCapacity, final float load_factor) {
        super(initialCapacity, load_factor);
    }
    
    public TByteHashSet(final int initial_capacity, final float load_factor, final byte no_entry_value) {
    ...
    public TByteHashSet(final Collection<? extends Byte> collection) {
        this(Math.max(collection.size(), 10));
        this.addAll(collection);
    }
    
    public TByteHashSet(final TByteCollection collection) {
        this(Math.max(collection.size(), 10));
        if (collection instanceof TByteHashSet) {
            final TByteHashSet hashset = (TByteHashSet)collection;
