...
public class TFloatArrayStack implements TFloatStack, Externalizable
{
    ...
    public TFloatArrayStack(final int capacity) {
        this._list = new TFloatArrayList(capacity);
    }
    
    public TFloatArrayStack(final int capacity, final float no_entry_value) {
        this._list = new TFloatArrayList(capacity, no_entry_value);
    }
    
    public TFloatArrayStack(final TFloatStack stack) {
        if (stack instanceof TFloatArrayStack) {
            final TFloatArrayStack array_stack = (TFloatArrayStack)stack;
    ...
    public float[] toArray() {
        final float[] retval = this._list.toArray();
        this.reverse(retval, 0, this.size());
        return retval;
    }
    
    public void toArray(final float[] dest) {
    ...
    private void reverse(final float[] dest, final int from, final int to) {
        if (from == to) {
            return;
        }
        if (from > to) {
    ...
    private void swap(final float[] dest, final int i, final int j) {
        final float tmp = dest[i];
        dest[i] = dest[j];
        dest[j] = tmp;
    }
    ...
    public boolean equals(final Object o) {
        ...
        final TFloatArrayStack that = (TFloatArrayStack)o;
