...
class CustomConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable
{
    ...
    CustomConcurrentHashMap(final MapMaker builder) {
    ...
    static class Segment<K, V> extends ReentrantLock
    {
        ...
        Segment(final CustomConcurrentHashMap<K, V> map, final int initialCapacity, final int maxSegmentSize) {
    ...
    static final class CleanupMapTask implements Runnable
    {
        ...
        public CleanupMapTask(final CustomConcurrentHashMap<?, ?> map) {
            this.mapReference = new WeakReference<CustomConcurrentHashMap<?, ?>>(map);
        }
        
        public void run() {
            final CustomConcurrentHashMap<?, ?> map = this.mapReference.get();
            if (map == null) {
    ...
    final class WriteThroughEntry extends AbstractMapEntry<K, V>
    {
        ...
        public V setValue(final V newValue) {
            final V oldValue = CustomConcurrentHashMap.this.put(this.key, newValue);
            this.value = newValue;
            return oldValue;
        }
    }
    ...
    abstract static class AbstractSerializationProxy<K, V> extends ForwardingConcurrentMap<K, V> implements Serializable
    {
        ...
        MapMaker readMapMaker(final ObjectInputStream in) throws IOException {
            ...
            final MapMaker mapMaker = new MapMaker().initialCapacity(size).setKeyStrength(this.keyStrength).setValueStrength(this.valueStrength).keyEquivalence(this.keyEquivalence).valueEquivalence(this.valueEquivalence).concurrencyLevel(this.concurrencyLevel);
