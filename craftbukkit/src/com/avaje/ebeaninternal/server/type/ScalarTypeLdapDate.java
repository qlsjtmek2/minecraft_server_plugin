...
public class ScalarTypeLdapDate<T> implements ScalarType<T>
{
    ...
    public ScalarTypeLdapDate(final ScalarType<T> baseType) {
    ...
    public Object toJdbcType(final Object value) {
        ...
        if (!(ts instanceof java.sql.Date)) {
            final String msg = "Expecting a java.sql.Date type but got " + value.getClass() + " value[" + value + "]";
            throw new PersistenceException(msg);
        }
        final java.sql.Date t = (java.sql.Date)ts;
        final SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss'Z'");
        return sdf.format(t);
    }
