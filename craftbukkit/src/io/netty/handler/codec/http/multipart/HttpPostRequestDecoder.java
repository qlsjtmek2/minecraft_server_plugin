...
public class HttpPostRequestDecoder
{
    ...
    public void offer(final HttpContent content) throws ErrorDataDecoderException {
        final ByteBuf chunked = content.data();
        if (this.undecodedChunk == null) {
            this.undecodedChunk = chunked;
        }
        else {
            this.undecodedChunk = Unpooled.wrappedBuffer(this.undecodedChunk, chunked);
        }
        ...
        while (!this.skipOneLine()) {
            ...
            if (contents[0].equalsIgnoreCase("Content-Disposition")) {
                ...
                for (int i = 2; i < contents.length; ++i) {
                    ...
                    Attribute attribute;
                    try {
                        attribute = this.factory.createAttribute(this.request, values[0].trim(), decodeAttribute(cleanString(values[1]), this.charset));
                    }
                ...
                if (contents[1].equalsIgnoreCase("multipart/mixed")) {
                    ...
                    for (int j = 1; j < contents.length; ++j) {
                        if (contents[j].toLowerCase().startsWith("charset")) {
                            ...
                            try {
                                attribute4 = this.factory.createAttribute(this.request, contents[0].trim(), decodeAttribute(cleanString(contents[j]), this.charset));
                            }
    ...
    private InterfaceHttpData getFileUpload(final String delimiter) throws ErrorDataDecoderException {
    ...
    private void readFileUploadByteMultipartStandard(final String delimiter) throws NotEnoughDataDecoderException, ErrorDataDecoderException {
        ...
        final ByteBuf buffer = this.undecodedChunk.slice(readerIndex, lastPosition - readerIndex);
        if (found) {
            try {
    ...
    private static String[] splitHeaderContentType(final String sb) {
        ...
        final int bStart = HttpPostBodyUtil.findNonWhitespace(sb, aEnd);
