...
public abstract class AbstractMemoryHttpData extends AbstractHttpData
{
    ...
    public void addContent(final ByteBuf buffer, final boolean last) throws IOException {
        if (buffer != null) {
            ...
            if (this.byteBuf == null) {
                ...
                this.byteBuf = Unpooled.wrappedBuffer(this.byteBuf, buffer);
            }
        }
        if (last) {
            this.completed = true;
    ...
    public void setContent(final File file) throws IOException {
        ...
        inputStream.close();
        byteBuffer.flip();
        this.byteBuf = Unpooled.wrappedBuffer(byteBuffer);
        this.size = newsize;
        this.completed = true;
    }
    ...
    public ByteBuf getChunk(final int length) throws IOException {
        ...
        final ByteBuf chunk = this.byteBuf.slice(this.chunkPosition, sliceLength);
    ...
    public boolean renameTo(final File dest) throws IOException {
        ...
        int written;
        for (fileChannel = outputStream.getChannel(), byteBuffer = this.byteBuf.nioBuffer(), written = 0; written < length; written += fileChannel.write(byteBuffer)) {}
