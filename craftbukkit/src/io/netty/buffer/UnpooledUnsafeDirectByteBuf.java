...
final class UnpooledUnsafeDirectByteBuf extends AbstractReferenceCountedByteBuf
{
    ...
    UnpooledUnsafeDirectByteBuf(final ByteBufAllocator alloc, final int initialCapacity, final int maxCapacity) {
        super(maxCapacity);
        this.leak = UnpooledUnsafeDirectByteBuf.leakDetector.open(this);
        if (alloc == null) {
    ...
    UnpooledUnsafeDirectByteBuf(final ByteBufAllocator alloc, final ByteBuffer initialBuffer, final int maxCapacity) {
        super(maxCapacity);
        this.leak = UnpooledUnsafeDirectByteBuf.leakDetector.open(this);
        if (alloc == null) {
            throw new NullPointerException("alloc");
    ...
    public ByteBuf copy(final int index, final int length) {
        this.checkIndex(index, length);
        final UnpooledUnsafeDirectByteBuf copy = (UnpooledUnsafeDirectByteBuf)this.alloc().directBuffer(length, this.maxCapacity());
        if (length != 0) {
            PlatformDependent.copyMemory(this.addr(index), copy.addr(0), length);
