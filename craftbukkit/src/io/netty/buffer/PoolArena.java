...
abstract class PoolArena<T>
{
    ...
    private void allocate(final PoolThreadCache cache, final PooledByteBuf<T> buf, final int reqCapacity) {
        ...
        if ((normCapacity & this.subpageOverflowMask) == 0x0) {
            int tableIdx;
            PoolSubpage<T>[] table;
            if ((normCapacity & 0xFFFFFE00) == 0x0) {
    ...
    synchronized void free(final PoolChunk<T> chunk, final long handle) {
        if (chunk.unpooled) {
    ...
    private int normalizeCapacity(final int reqCapacity) {
        if (reqCapacity < 0) {
        ...
        if ((reqCapacity & 0xFFFFFE00) != 0x0) {
            int normalizedCapacity;
            for (normalizedCapacity = 512; normalizedCapacity < reqCapacity; normalizedCapacity <<= 1) {}
