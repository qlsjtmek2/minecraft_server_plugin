...
public final class Unpooled
{
    ...
    public static <T> MessageBuf<T> messageBuffer(final int initialCapacity) {
        return new DefaultMessageBuf<T>(initialCapacity);
    }
    
    public static <T> MessageBuf<T> messageBuffer(final int initialCapacity, final int maxCapacity) {
        return new DefaultMessageBuf<T>(initialCapacity, maxCapacity);
    }
    
    public static <T> MessageBuf<T> wrappedBuffer(final Queue<T> queue) {
        if (queue instanceof MessageBuf) {
            return (MessageBuf<T>)(MessageBuf)queue;
        }
    ...
    public static ByteBuf wrappedBuffer(final ByteBuffer buffer) {
        ...
        if (PlatformDependent.hasUnsafe()) {
            ...
            if (buffer.isReadOnly()) {
                return new ReadOnlyDirectByteBuf(Unpooled.ALLOC, buffer);
            }
        ...
        switch (arrays.length) {
            ...
            default: {
                ...
                if (!components.isEmpty()) {
                    return new DefaultCompositeByteBuf(Unpooled.ALLOC, false, maxNumComponents, components);
                }
                break;
            }
        }
        ...
        switch (buffers.length) {
            ...
            default: {
                for (final ByteBuf b : buffers) {
                    if (b.isReadable()) {
                        return new DefaultCompositeByteBuf(Unpooled.ALLOC, false, maxNumComponents, buffers);
                    }
                }
                break;
            }
    ...
    public static CompositeByteBuf compositeBuffer(final int maxNumComponents) {
        return new DefaultCompositeByteBuf(Unpooled.ALLOC, false, maxNumComponents);
    }
    ...
    private static ByteBuf copiedBuffer(final CharBuffer buffer, final Charset charset) {
        final ByteBuffer dst = BufUtil.encodeString(buffer, charset);
        final ByteBuf result = wrappedBuffer(dst.array());
